...
function VisitTypeBadge({ type }: { type: string | null }) {
  if (!type) return null;
  const cls =
    VISIT_TYPE_COLORS[type] ?? "bg-stone-100 text-stone-700 border-stone-200";
  return (
    <span
...
function CooperationBadge({ level }: { level: string | null }) {
  ...
  const cls =
    COOPERATION_COLORS[level] ?? "bg-stone-100 text-stone-700 border-stone-200";
  return (
    <span
      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px]
      font-semibold uppercase tracking-wide border ${cls}`}
...
function DetailRow({
  ...
  return (
    <div className="grid grid-cols-[9rem_1fr] gap-x-3 py-2.5 border-b border-stone-100 last:border-0">
      <dt className="text-xs font-semibold text-stone-500 uppercase tracking-wide leading-5">
        {label}
      </dt>
...
export default function HomeVisitationPage() {
      ...
      .then(([visits, rawResidents, safehouses]) => {
        ...
        const mapped: ResidentDisplay[] = rawResidents
          ...
          .map((r) => ({
            ...
            safehouseName:
              (r.safehouseId != null ? shMap.get(r.safehouseId) : undefined) ??
              "Unknown",
  ...
  function openAdd() {
    ...
    setForm(
      makeEmptyForm(
        ...
        selectedResident.assignedSocialWorker,
      ),
    );
  ...
  function openView(v: HomeVisitation) {
    setSelectedVisit(v);
    setModal("view");
  }
  function openDelete(v: HomeVisitation) {
    setSelectedVisit(v);
    setModal("delete");
  }
  ...
  function confirmDelete() {
    if (selectedVisit) {
      deleteVisitation(selectedVisit.visitationId).then(() => {
        setVisitations((prev) =>
          prev.filter((v) => v.visitationId !== selectedVisit.visitationId),
        );
      });
    }
    setModal(null);
  ...
  return (
    <div className="px-4 sm:px-6 py-6 max-w-7xl mx-auto flex flex-col lg:flex-row gap-6">
      ...
      {/* ------------------------------------------------------------------ */}
      <div
        className={`w-full lg:w-80 shrink-0 ${mobileShowDetail ? "hidden lg:block" : ""}`}
      >
        ...
        <div className="space-y-2 lg:max-h-[70vh] overflow-y-auto pr-1">
          {filteredResidents.map((r) => (
            ...
            >
              <p className="text-sm font-semibold font-mono text-stone-900">
                {r.caseLabel}
              </p>
      ...
      {/* ------------------------------------------------------------------ */}
      <div
        className={`flex-1 min-w-0 ${!mobileShowDetail ? "hidden lg:block" : ""}`}
      >
        {/* Mobile back button */}
        {mobileShowDetail && (
          <button
        ...
        {selectedResident ? (
          <>
            {/* Resident header */}
            <div
              className="bg-white rounded-xl border border-stone-200 shadow-sm p-5 mb-5
              flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4"
            >
              <div>
                <p className="font-mono text-base font-bold text-stone-900">
                  {selectedResident.caseLabel}
                </p>
                <p className="text-sm text-stone-500 mt-0.5">
                  {selectedResident.safehouseName} ·{" "}
                  {selectedResident.assignedSocialWorker}
                </p>
                <p className="text-xs text-stone-400 mt-1">
                  {residentVisitations.length} visit
                  {residentVisitations.length !== 1 ? "s" : ""} on record
                </p>
              </div>
              <button
                type="button"
            ...
            {residentVisitations.length === 0 ? (
              <div className="bg-white rounded-xl border border-stone-200 shadow-sm p-12 text-center">
                <p className="text-stone-400 text-sm">
                  No visits recorded for this resident.
                </p>
                <button
                  type="button"
                  onClick={openAdd}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2 text-sm">
                      ...
                      {visit.familyCooperationLevel && (
                        <div>
                          ...
                          <div className="mt-1">
                            <CooperationBadge
                              level={visit.familyCooperationLevel}
                            />
                          </div>
                        </div>
                      )}
          ...
          <div
            className="hidden lg:flex flex-col items-center justify-center h-96 bg-white rounded-xl
          ...
          >
            ...
            <p className="text-xs text-stone-400 mt-1">
              Choose a resident from the list to view or log home visits
            </p>
          </div>
        )}
      </div>
            ...
            <form onSubmit={saveForm} className="px-6 py-5 space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setForm((f) => ({ ...f, socialWorker: e.target.value }))
                    }
                    className={inputCls}
                  >
                    {SOCIAL_WORKERS.map((sw) => (
                ...
                <div>
                  ...
                  <input
                    id="hv-location"
                    type="text"
                    placeholder="e.g. Family home — Quezon City"
              ...
              <div>
                ...
                <input
                  id="hv-familyMembers"
                  type="text"
                  placeholder="e.g. Mother, older brother"
              ...
              <div>
                ...
                <textarea
                  id="hv-observations"
                  rows={3}
                  placeholder="Describe home environment, family dynamics, welfare of the resident's support network…"
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, visitOutcome: e.target.value }))
                  }
                  className={inputCls}
                >
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  onChange={(e) =>
                    setForm((f) => ({
                      ...f,
                      followUpNeeded: e.target.checked ? "True" : "False",
                    }))
                  }
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="relative w-full max-w-2xl bg-white rounded-2xl shadow-xl my-6">
            ...
            <dl className="px-6 py-4">
              <DetailRow label="Social Worker">
                {selectedVisit.socialWorker ?? "—"}
              </DetailRow>
              <DetailRow label="Location">
                {selectedVisit.locationVisited ?? "—"}
              </DetailRow>
              <DetailRow label="Family Present">
                {selectedVisit.familyMembersPresent ?? "—"}
              </DetailRow>
              ...
              <DetailRow label="Cooperation">
                {selectedVisit.familyCooperationLevel ? (
                  <CooperationBadge
                    level={selectedVisit.familyCooperationLevel}
                  />
                ) : (
                  "—"
                )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="w-full max-w-sm bg-white rounded-2xl shadow-xl p-6">
            ...
            <p className="text-sm text-stone-600 mb-6">
              ...
              <span className="font-semibold">{selectedVisit.visitType}</span>{" "}
              visit record. This action cannot be undone.
            </p>
